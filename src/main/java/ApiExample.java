    ...
    public static void main(String[] args)
       ...
       /* YouTube youtubeService = SubExtractor.getService();
        ...
        if ("text".equals(args[1])) {
            ...
            String keyword = "java";
            ArrayList<Timing> keywordTimming = new ArrayList<>();
            ArrayList<Timing> keywordTimecodes = new ArrayList<>();
            for (int i = 0; i < subtitles.size(); i++) {
                if(subtitles.get(i).contains(keyword)) {
                    keywordTimming.add(timming.get(i));
                }
            }
            final long TIME_WINDOW = 120000;
            for(Timing time : keywordTimming) {
                if(keywordTimecodes.isEmpty()) {
                    keywordTimecodes.add(time);
                } else {
                    if (time.getBegin().toMillis() - keywordTimecodes.get(keywordTimecodes.size() - 1).getBegin().toMillis() <= TIME_WINDOW) {
                        keywordTimecodes.get(keywordTimecodes.size() - 1).setEnd(time.getEnd());
                    }
                    else {
                        keywordTimecodes.add(time);
                    }
            ...
            System.out.println(keywordTimming);
            System.out.println("End");
            System.out.println(keywordTimecodes);
